...
public abstract class TestDictionary {
  ...
  /**
   * Tests if the put method works and remove at the same time.
   */
    @Test
    public void testPut() {
      d.put("Hi", 5);
      d.put("Hiya",1);
      d.remove("Hi");
      assertTrue("clear() failed for non-empty dictionary ", d.size() == 1);
    }

  /**
   * tests if clear method works with a lot of elements in dictionary.
   */
    @Test
    public void testClearWithMoreElems() {
  ...
  /**
   * My test for size change.
   */
    @Test
    public void testSizeWithOneLessCat() {
      List<String> cats = Arrays.asList("Growltiger", "Rum Tum Tugger",
          "Jellicles", "Mungojerrie", "Rumpelteazer");
      for (String cat : cats) {
        d.put(cat, cat.hashCode());
      ...
      d.remove("Growltiger");
      assertEquals("size() failed for multiple cats", cats.size()-1, d.size());
    }
